...
// The first thing the orchestrator (god) is told on a fresh spawn — orient him and put
// him to work running the floor. Kept terse and action-oriented.
const INITIAL_GOD_PROMPT = [
  "You're online as the Orchestrator of this agent fleet. Get oriented, then start running the fleet:",
 ...
 *   1. spawns the god agent into the orchestrator's room when none is running,
...
export function useHive(config: HarnessConfig | null): void {
  ...
  // never drains, so the "newest" id was frozen on it: the orchestrator was nudged once per
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      const res = await window.cth.spawnPty({
        ...
        cols: 100,
        rows: 30,
        // Restore the orchestrator's prior conversation across an app restart. His session
        ...
        resume: true,
        hive: { id: GOD_ID, name: 'Orchestrator', provider: godProvider, cwd: config.harnessHome!, isGod: true, role: 'orchestrator (god)' }
      });
      ...
      const god: Agent = {
        ...
        name: 'Orchestrator',
        accent: 'lemon',
        description: 'god — runs the fleet, triages requests, escalates only critical calls to you',
        project: 'hive',
        tmuxTarget: '',
        cwd: config.harnessHome!,
        status: 'idle',
        action: 'running the fleet',
        progress: 0,
        currentStation: 'desk',
        ptyId: GOD_PTY,
      ...
      // Kick the orchestrator off once his TUI is up. Always re-enable remote control so
      ...
      // a RESUMED the orchestrator already has his full context and must not be re-oriented
      ...
      // the orchestrator until he's settled. The live-PTY branch above skips this entirely.
      const resumedGod = res.resumed === true;
      bootGraceUntil.current[GOD_ID] = Date.now() + BOOT_GRACE_MS;
      void (async () => {
        try {
          const remoteCommand = remoteControlCommandForProvider(godProvider, 'Orchestrator');
          if (remoteCommand) {
  ...
  useEffect(() => {
    return window.cth.onHiveHookEvent((e) => {
      ...
      if (e.event === 'PreCompact') {
        ...
        // Only (1) is a real "needs you". Treating (2) as blocked made the orchestrator
  ...
  // 5) Pipe inbound Slack messages into the orchestrator's queue. The main-process Slack
    ...
    const revive = async (deadId: string): Promise<void> => {
      ...
      try {
        ...
        const hive = a.isGod
          ...
          : a.isAssistant
          ? { id: a.id, name: a.name, cwd, provider, isAssistant: true, role: "the orchestrator's prep assistant" }
