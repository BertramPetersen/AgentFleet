...
export interface Agent {
  ...
  lastPrompt?: string;
  /** the orchestrator ("god") agent — seated in the orchestrator's room, runs the floor */
  isGod?: boolean;
  /** the orchestrator's prep assistant — send-only; enriches prompts and forwards them to
...
/** Lifecycle of the orchestrator (god agent) bootstrap on launch.
 ...
 *  don't see the "add agent" prompt before the orchestrator has clocked in. */
...
interface State {
   ...
   *  secret broker, never the store). Gates the Realtime voice voice toggle the
...
export const useStore = create<State>((set) => ({
  ...
  addAgent: (agent) =>
    set((s) => {
      ...
      // GOD enters at the HEAD, everyone else at the tail. The orchestrator's position was
