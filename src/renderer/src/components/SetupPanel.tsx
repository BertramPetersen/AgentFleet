 ...
 * the user's machine and can need a password, so the button SEEDS the orchestrator's
...
export function SetupPanel({ onDone }: { onDone?: () => void } = {}) {
  ...
  // Only ESSENTIALS are handed to the orchestrator. Installing all eight engine CLIs
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 220, fontSize: 12, color: 'var(--cth-ink-700)', lineHeight: 1.5 }}>
          {missingEssential.length
            ? <>The orchestrator can install the {missingEssential.length} missing recommended {missingEssential.length === 1 ? 'tool' : 'tools'} for you. This fills in its dispatch box — nothing runs until you press dispatch.</>
        ...
        >
          <span style={{ display: 'inline-flex', gap: 4, alignItems: 'center' }}>
            <Icon name="sparkle" /> ask the orchestrator to set up everything
          </span>
        </PixelButton>
      </div>
