...
function ExchangeCard({
  ...
  const tail = (() => {
    ...
    return 'No reply yet. The orchestrator has this one.';
  })();
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={{ ...uiText, fontSize: 11, lineHeight: '16px', color: 'var(--cth-ink-700)' }}>
            {pending.kind === 'directive'
              ? 'Approve and this goes to the orchestrator, who will put the hive to work on it. Reject and it is dropped — nothing runs.'
              : 'Approve and the orchestrator reads this. Reject and it is dropped — nothing runs.'}
          </div>
          <div style={{ display: 'flex', gap: 6 }}>
            <PixelButton
              ...
              title="Send this message through to the orchestrator"
            >
...
const SECTIONS: { key: Source; label: string; blurb: string }[] = [
  {
    key: 'webhook',
    label: 'Webhooks',
    blurb: 'Everything posted to your webhook endpoints, next to what the orchestrator sent back.'
  },
  {
    key: 'org',
    label: 'Organization',
    blurb: 'Messages from your teammates’ clone nodes, next to what the orchestrator sent back.'
  }
];
        ...
        {exchanges.length === 0 ? (
          source === 'org' ? (
            ...
            <EmptyState
              ...
              body={'When something posts to one of your endpoints, it lands here with the orchestrator’s '
